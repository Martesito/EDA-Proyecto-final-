...
static std::vector<int> path_internal_from_dijkstra(const DijkstraResult& r,
                                                     int src, int dst) {
    return reconstruct_path(r.parent, src, dst);
}

static int run_module_b(const std::string& dataset, std::vector<int>* path_q01_out = nullptr,
                        std::vector<int>* path_q06_out = nullptr, Graph* g_out = nullptr) {
    ...
    for (const Query& q : QUERIES) {
        ...
        if (q.id == "Q01") {
            std::vector<int> path = path_internal_from_dijkstra(dr, src, dst);
            std::ofstream pf("results/camino_Q01.txt");
            pf << "# Camino reconstruido Q01: " << q.src_orig << " -> " << q.dst_orig << "\n";
            pf << "# Longitud (nodos): " << path.size() << "\n";
            for (int v : path) pf << g.get_original(v) << "\n";
            if (path_q01_out) *path_q01_out = path;
        }
        if (q.id == "Q06") {
            std::vector<int> path = path_internal_from_dijkstra(dr, src, dst);
            std::ofstream pf("results/camino_Q06.txt");
            pf << "# Camino reconstruido Q06: " << q.src_orig << " -> " << q.dst_orig << "\n";
            pf << "# Longitud (nodos): " << path.size() << "\n";
            for (int v : path) pf << g.get_original(v) << "\n";
            if (path_q06_out) *path_q06_out = path;
        }
    }
